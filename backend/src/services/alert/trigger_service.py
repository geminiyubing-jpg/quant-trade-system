...
from sqlalchemy.orm import Session
 ...
 class AlertTriggerService:
  ...
  """启动服务"""
        ...
        async def stop(self):  """停止服务"""
        self._running = False
        if self._task:
            self._task.cancel()
            logger.info("🛑 预警触发服务已停止")

        async def _check_loop(self):  """检查循环"""
        while self._running:
  try:
            try:
        ...
        async def _check_alerts(self):  """检查所有活跃的预警"""
        if not self._db:
            return
  ...
  .all()
        ...
        async def _check_single_alert(self, alert: PriceAlert):  """检查单个预警"""
        # 获取当前价格
 ...
 if stock.current_price else Decimal("0")
  ...
  percent_change = (current_price - alert.current_price) / alert.current_price * 100 if percent_change >= alert.target_value:
                triggered = True
                trigger_value = alert.target_value
        ...
        await ws_manager.broadcast_to_user(
            alert.user_id,
            {
                ...
                "data": {
                    ...
                    "triggered_at": alert.triggered_at.isoisoformat(),
                    "message": f"预警触发: {alert.symbol} {alert.alert_type.value}",
                }
            }
        )

        logger.info(f"✅ 飘预警通知已发送给用户 {alert.user_id}")

        async def _send_notification(self, alert: PriceAlert, current_price: Decimal):  """发送外部通知"""
        try:
            # 获取用户设置
            user = self._db.query(User).filter(User.id == alert.user_id).first()
            if not user:
                return
            ...
            quiet_start = alert_settings.get("quiet_hours_start", 22)
            quiet_end = alert_settings.get("quiet_hours_end", 23)

            if quiet_start and not None and quiet_end is not None:
                # 不在静音时段, 发送浏览器通知
                if alert_settings.get("browser_notification", True):
                ...
                # 发送声音提醒
                if alert_settings.get("sound_enabled", True):
                    await notification_service.play_alert_sound()
        except Exception as e:
